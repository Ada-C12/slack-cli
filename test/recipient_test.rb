...
describe 'class recipient' do
  describe 'initialize' do 
    let(:recipient){
    Slack::Recipient.new(id: "CN9FFLVEK", name: "random")
  }
...
describe 'self.get' do    
  ...
  it 'should raise a Slack_Api_Error if the response code is not 200 or response is false' do
    VCR.use_cassette("response_code_error_cassette") do
      expect{
      Slack::Recipient.get("https://us1.locationiq.org/v1/search.php?", {token: ENV["SLACK_TOKEN"]})
    }.must_raise Slack::Slack_Api_Error
  end
  
  VCR.use_cassette("false_response_cassette") do
    expect{
    Slack::Recipient.get("https://slack.com/api/users.list", {token: "XXXXXXXX"})
  }.must_raise Slack::Slack_Api_Error
end
end
end
...
describe "send_message" do 
  let(:channel_recipient){
  Slack::Recipient.new(id: "CN9FFLVEK", name: "random")
}

let(:user_recipient){
Slack::Recipient.new(id: "USLACKBOT", name: "slackbot")
}
...
it "must successfully sending messages to channels" do
  VCR.use_cassette("channel_chat_post_cassette") do
...
it "must successfully sending messages to individual recipients" do
  VCR.use_cassette("user_chat_post_cassette") do
