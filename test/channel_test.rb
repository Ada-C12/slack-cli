...
describe "### TESTING CHANNEL ###" do
  let (:response) { Channel.get_raw_data }
...
# it "Does get member count work?" do
...
# it "Does get get channel ids work?" do
...
# end
  ...
  # it "Does channels_in_giant_hash work?" do
  #   giant_hash = Channel.channels_in_giant_hash
  #   assert(giant_hash.length == 3)
  ...
  #   index = 0
  #   ap giant_hash
  #   giant_hash.each do |key, values|
  #     assert(key == ids[index])
  #     assert(values[:name] == names[index])
  #     assert(values[:topic] == topics[index])
  #     assert(values[:member_count] == 2)
  #     index += 1
  #   end
  # end
